...
| 能力 | 实现 |
|------|------|
| 远程升级 / 安装 / 卸载 | `downloadAndInstallApk(url, sha256?)` + FileProvider 安装；**root 设备优先 `pm install -r -g` 静默安装（免手动确认）**，3 次重试，可选 SHA-256 校验 |
| 截屏上报 | `capture()`：**API26+ 用 PixelCopy 抓合成帧（含硬解视频，不再黑屏）**，老设备降级 `webView.draw()` → base64 → 上传 |
...
| 开机自启 | BootReceiver 监听 BOOT_COMPLETED；**首次启动自动点亮屏幕并越过锁屏**（FLAG_TURN_SCREEN_ON / DISMISS_KEYGUARD / SHOW_WHEN_LOCKED） |
| 全屏沉浸 | SYSTEM_UI_FLAG_IMMERSIVE_STICKY，拦截返回键防误退 |
| 硬件指纹 | `getHardwareInfo()` 上报 mac/serial/model/分辨率，作为注册幂等键 |
| 联网健壮性 | WebView `LOAD_CACHE_ELSE_NETWORK` + 应用缓存（服务端重启不白屏）；连不上时显示「正在重连」覆盖层 + 15s 自动重试 |
| 网络变化自愈 | ConnectivityManager 监听，重连后通知引擎 `window.__onNetworkChange` 重开 SSE 指令流 + 立即刷新清单 |
| 健康遥测 | `getNativeStatus()` 上报 **真实 CPU 温度（读 thermal_zone）/ 电池电量 / 内存 / CPU / 崩溃次数** |
| 安全 | WebView 远程调试仅在 debug 构建开启（release 关闭） |
