...
class KioskGuardService : Service() {
    ...
    private fun buildNotification(): Notification {
        // getLaunchIntentForPackage 在 compileSdk 34 下返回 Intent?（可能为空），
        // 必须判空后再传给 PendingIntent.getActivity（其参数要求非 null Intent）。
        val flag = PendingIntent.FLAG_UPDATE_CURRENT or
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) PendingIntent.FLAG_IMMUTABLE else 0
        val launchIntent = packageManager.getLaunchIntentForPackage(packageName)
        val pendingIntent = if (launchIntent != null) {
            PendingIntent.getActivity(this, 0, launchIntent, flag)
        } else null
